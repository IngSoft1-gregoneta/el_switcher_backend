...
def test_match_not_found():
    ...
    x = 0
    y = 0
    response = client.put(f"/block_figure/{not_room_id}/{player.player_name}/{other_player.player_name}/{card_index}/{x}/{y}")
...
def test_player_not_found():
    ...
    other_player = match.get_player_by_name(match.players[1].player_name)
    response = client.put(f"/block_figure/{room_id}/{no_player_name}/{other_player.player_name}/{card_index}/{x}/{y}")
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_other_player_not_found():
    ...
    response = client.put(f"/block_figure/{room_id}/{player.player_name}/{no_other_player_name}/{card_index}/{x}/{y}")
    assert response.status_code == status.HTTP_404_NOT_FOUND
