...
class MatchHandler:
    turn_timers_task: Dict[UUID, asyncio.Task] = {}
    timers: Dict[UUID, datetime] = {}
        ...
        try:
            ...
            if target_player.has_turn is False:
                raise HTTPException(
                    ...
                    detail="Player has not the turn",
                )
    ...
    async def do_parcial_mov(
        ...
        if not player.has_turn:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Player has not turn"
            )
        ...
        if mov_card.is_used:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Card is used"
            )
    ...
    async def revert_mov(self, match_id: UUID, player_name: str):
        ...
        if new_match.state == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot go back beyond the initial state",
            )
        state_handler.remove_last_parcial_match(match_id)

    async def discard_fig(
        self, match_id: UUID, player_name: str, card_index: int, x: int, y: int
    ):
        match = state_handler.get_parcial_match(match_id)
        if match is None:
            raise HTTPException(
        ...
        if not fig_card.is_visible:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Card is not visible"
            )
        ...
        if switcher.pos_in_range(x, y):
            ...
            if tile.tile_in_figure == fig_card.fig_type:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Fig card not match with figure",
                )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid positions"
            )
    ...
    async def block_fig(
        ...
        if other_player is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Other player not found"
            )
        if not player.has_turn:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Player has not turn"
        ...
        for fig_card in other_player.fig_cards:
            if fig_card.is_blocked:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Other player already has blocked cards",
                )
        if switcher.pos_in_range(x, y):
            tile = new_match.board.tiles[switcher.coordinates_to_index(x, y)]
