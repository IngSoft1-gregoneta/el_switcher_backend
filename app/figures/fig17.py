...
def fig17_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    board_out = board
    if board is None: 
    ...
    if (fig17_verifications(center_x, center_y)):
        ...
        fig17_borders_verifications(center_x, center_y, board, color):
            board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig17.value
            board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig17.value
            board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_in_figure = FigType.fig17.value
            board.tiles[figure_detector.coordinates_to_index(left[0], left[1])].tile_in_figure = FigType.fig17.value
            board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fig17.value
    return board_out
...
def fig17_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
