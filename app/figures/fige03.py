...
def fige03_detector(board: Board, x: int, y: int) -> Board:
    board_out = board
    board_out = fige03_rot1_detector(board_out, x, y)
    board_out = fige03_rot2_detector(board_out, x, y)
    return board_out

def fige03_rot1_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    if board is None: 
    ...
    if (fige03_rot1_verifications(center_x, center_y)):
        ...
        fige03_rot1_borders_verifications(center_x, center_y, board, color):
            board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(up_left[0], up_left[1])].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fige03.value
    return board
...
def fige03_rot1_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
...
def fige03_rot2_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    board_out = board
    if board is None: 
    ...
    if (fige03_rot2_verifications(center_x, center_y)):
        ...
        board.tiles[figure_detector.coordinates_to_index(up_left[0], up_left[1])].tile_color == color and \
        board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_color == color and \
        fige03_rot2_borders_verifications(center_x, center_y, board, color):
            board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(up_left[0], up_left[1])].tile_in_figure = FigType.fige03.value
            board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fige03.value
...
def fige03_rot2_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
