...
def fig05_detector(board: Board, x: int, y: int) -> Board:
    board_out = board
    board_out = fig05_rot1_detector(board_out, x, y)
    board_out = fig05_rot2_detector(board_out, x, y)
    return board_out

def fig05_rot1_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    board_out = board
    if board is None: 
    ...
    if (fig05_rot1_verifications(center_x)):
        ...
        fig05_rot1_borders_verifications(center_x, center_y, board, color):
            board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(left[0], left[1])].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(left2[0], left2[1])].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(right2[0], right2[1])].tile_in_figure = FigType.fig05.value
    return board_out

def fig05_rot1_verifications(center_x: int):
    return center_x-2>=0 and center_x+2<figure_detector.columns

def fig05_rot1_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
    ...
    border12[1] >= 0 and border12[1]<figure_detector.columns: 
        valid = valid and \
        board.tiles[figure_detector.coordinates_to_index(border12[0], border12[1])].tile_color != color   
    return valid

def fig05_rot2_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    board_out = board
    if board is None: 
    ...
    if (fig05_rot2_verifications(center_y)):
        ...
        board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_color == color and \
        board.tiles[figure_detector.coordinates_to_index(right2[0], right2[1])].tile_color == color and \
        fig05_rot2_borders_verifications(center_x, center_y, board, color):
            board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(left[0], left[1])].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(left2[0], left2[1])].tile_in_figure = FigType.fig05.value
            board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fig05.value
...
def fig05_rot2_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
