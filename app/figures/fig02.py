...
def fig02_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    match_out = match
    match_out = fig02_rot1_detector(match_out, x, y)
    match_out = fig02_rot2_detector(match_out, x, y)
    return match_out

def fig02_rot1_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
    ...
    if (fig02_rot1_verifications(center_x, center_y)):
        ...
        fig02_rot1_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up_left[0], up_left[1])].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(right2[0], right2[1])].tile_in_figure = FigType.fig02.value
    return match_out
...
def fig02_rot1_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
...
def fig02_rot2_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
    ...
    if (fig02_rot2_verifications(center_x, center_y)):
        ...
        match_out.board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_color == color and \
        match_out.board.tiles[figure_detector.coordinates_to_index(right2[0], right2[1])].tile_color == color and \
        fig02_rot2_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up_left[0], up_left[1])].tile_in_figure = FigType.fig02.value
            match_out.board.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fig02.value
...
def fig02_rot2_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
