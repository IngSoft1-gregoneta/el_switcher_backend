...
def fige02_detector(board: Board, x: int, y: int) -> Board:
    center_x = x
    center_y = y
    board_out = board
    if board is None: 
    ...
    if (fige02_verifications(center_x, center_y)):
        ...
        board_out.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_color == color and \
        fige02_borders_verifications(center_x, center_y, board, color):
            board_out.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fige02.value
            board_out.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fige02.value
            board_out.tiles[figure_detector.coordinates_to_index(up_right[0], up_right[1])].tile_in_figure = FigType.fige02.value
            board_out.tiles[figure_detector.coordinates_to_index(right[0], right[1])].tile_in_figure = FigType.fige02.value
    return board_out

def fige02_verifications(center_x: int, center_y: int):
    return center_y-1>=0 and center_x+1<figure_detector.columns

def fige02_borders_verifications(center_x: int, center_y: int, board: Board, color: TileColor):
    valid = True
