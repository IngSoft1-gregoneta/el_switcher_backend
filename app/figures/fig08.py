...
def fig08_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    match_out = match
    match_out = fig08_rot1_detector(match_out, x, y)
    match_out = fig08_rot2_detector(match_out, x, y)
    match_out = fig08_rot3_detector(match_out, x, y)
    match_out = fig08_rot4_detector(match_out, x, y)
    return match_out

def fig08_rot1_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="match not found")
    if (fig08_rot1_verifications(center_x, center_y)):
        ...
        fig08_rot1_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up2[0], up2[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(down_right[0], down_right[1])].tile_in_figure = FigType.fig08.value
    return match_out

def fig08_rot1_verifications(center_x: int, center_y: int):
    return center_y-2>=0 and center_y+1<figure_detector.columns and center_x+1<figure_detector.columns

def fig08_rot1_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
...
def fig08_rot2_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="match not found")
    if (fig08_rot2_verifications(center_x, center_y)):
        color = match_out.board.tiles[figure_detector.coordinates_to_index(x, y)].tile_color
        ...
        match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_color == color and \
        match_out.board.tiles[figure_detector.coordinates_to_index(down_right[0], down_right[1])].tile_color == color and \
        fig08_rot2_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up2[0], up2[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_in_figure = FigType.fig08.value
...
def fig08_rot2_verifications(center_x: int, center_y: int):
    return center_y-1>=0 and center_x-2>=0 and center_x+1<figure_detector.columns

def fig08_rot2_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
...
def fig08_rot3_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="match not found")
    if (fig08_rot3_verifications(center_x, center_y)):
        color = match_out.board.tiles[figure_detector.coordinates_to_index(x, y)].tile_color
        up = (center_x, center_y+1)
        up2 = (center_x, center_y+2)
        down = (center_x, center_y-1)
        ...
        match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_color == color and \
        match_out.board.tiles[figure_detector.coordinates_to_index(down_right[0], down_right[1])].tile_color == color and \
        fig08_rot3_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up2[0], up2[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_in_figure = FigType.fig08.value
...
def fig08_rot3_verifications(center_x: int, center_y: int):
    return center_y-1>=0 and center_y+2<figure_detector.columns and center_x-1>=0

def fig08_rot3_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
    border1 = (center_x, center_y+3)
    border2 = (center_x-1, center_y+2)
    border3 = (center_x-1, center_y+1)
...
def fig08_rot4_detector(match: MatchOut, x: int, y: int) -> MatchOut:
    center_x = x
    center_y = y
    match_out = match
    if match is None: 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="match not found")
    if (fig08_rot4_verifications(center_x, center_y)):
        color = match_out.board.tiles[figure_detector.coordinates_to_index(x, y)].tile_color
        up = (center_x+1, center_y)
        up2 = (center_x+2, center_y)
        down = (center_x-1, center_y)
        ...
        match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_color == color and \
        match_out.board.tiles[figure_detector.coordinates_to_index(down_right[0], down_right[1])].tile_color == color and \
        fig08_rot4_borders_verifications(center_x, center_y, match, color):
            match_out.board.tiles[figure_detector.coordinates_to_index(center_x, center_y)].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up[0], up[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(up2[0], up2[1])].tile_in_figure = FigType.fig08.value
            match_out.board.tiles[figure_detector.coordinates_to_index(down[0], down[1])].tile_in_figure = FigType.fig08.value
...
def fig08_rot4_verifications(center_x: int, center_y: int):
    return center_y+1<figure_detector.columns and center_x-1>=0 and center_x+2<figure_detector.columns

def fig08_rot4_borders_verifications(center_x: int, center_y: int, match: MatchOut, color: TileColor):
    valid = True
    border1 = (center_x+3, center_y)
    border2 = (center_x+2, center_y+1)
    border3 = (center_x+1, center_y+1)
