...
from config.Tables_initializer import Room
from config.repositorymanager import Session
from models.room import RoomIn, RoomOut


class RoomRepository:
    def create_room(self, new_room: RoomIn, User_id: UUID):
        db = Session()
        try:
            new_id = uuid1()
            ...
            roombd = Room(
                ...
                room_id=str(new_id),
                players_expected=new_room.players_expected,
                players_UUIDs = json.dumps(roomOut.players_UUIDs),
                owner_name=new_room.owner_name,
