...
class MatchOut(BaseModel):
    ...
    def create_players(self, match_id: UUID) -> List[Player]:
        ...
        for player_name in players_names:
            fig_cards, white_deck, blue_deck = self.create_fig_cards(
                white_per_player, blue_per_player, white_deck, blue_deck
            )
            for i in range(3):
    ...
    def create_fig_cards(
        ...
        blue_per_player: int,
        white_deck: List[FigType],
        blue_deck: List[FigType],
    ) -> Tuple[List[FigCard], List[FigType], List[FigType]]:
        fig_cards = []
    ...
    def create_mov_cards() -> List[MovCard]:
        mov_cards = []
        for i in range(3):
            new_mov_card = MovCard(mov_type=random.choice(list(MovType)), is_used=False)
...
class MatchRepository:
    ...
    def get_match_by_id(self, match_id_selected: UUID) -> MatchOut:
        ...
        try:
            matchdb = (
                db.query(Match)
                .filter(Match.match_id == str(match_id_selected))
                .one_or_none()
            )
            if not matchdb:
                return None
            ...
            board_db = Board.model_construct(
                blocked_color=blocked_color_data, tiles=tileslist
            )
            ...
            match = MatchOut.model_construct(
                match_id=str(match_id_selected),
                state=0,
                board=board_db,
                players=players_db,
            )
    ...
    def end_turn(self, match: MatchOut, player_name: str):
        for i in range(len(match.players)):
            if match.players[i].player_name == player_name:
                ...
                match.players[(i + 1) % len(match.players)].has_turn = (
                    True  # next player
                )
                match.state = 0
        self.update_match(match)
    ...
    def update_match(self, match_: MatchOut):
        ...
        try:
            matchdb = (
                db.query(Match)
                .filter(Match.match_id == str(match.match_id))
                .one_or_none()
            )
