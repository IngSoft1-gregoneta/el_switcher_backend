...
class MovCard(BaseModel):
    ...
    def __init__(self, mov_type: MovType = None, mov_status: MovStatus = "Held"):
        if mov_type is None:
            mov_type = self.create_random_mov()
        super().__init__(mov_type=mov_type, mov_status=mov_status)
    ...
    def confirm_mov_card(self):
        if self.is_used:
            ...
            raise ValueError("Cannot confirm a card that has not been used.")
