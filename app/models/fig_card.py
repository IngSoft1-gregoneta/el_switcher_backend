...
class FigCard(BaseModel):
    ...
    def validate_card(self):
        if not self.is_valid_color():
            raise ValueError(
                f"Invalid card, {self.card_color} is not a valid CardColor"
            )
        if not self.is_valid_fig_type():
            raise ValueError(
                f"Invalid fig type, {self.fig_type} is not a valid FigType"
            )
        if not self.is_valid_card():
            raise ValueError(
                f"Invalid card color, {self.card_color.value} card with figure {self.fig_type.value} is not allowed."
            )
    ...
    def is_valid_card(self):
        ...
        return (self.card_color == CardColor.WHITE and is_white_fig) or (
            self.card_color == CardColor.BLUE and is_blue_fig
        )
